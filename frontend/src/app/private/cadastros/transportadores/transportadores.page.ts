import { TransportadorModel } from 'src/app/models/transportador.model';
...
export class TransportadoresPage implements OnInit {
  ...
  public lista : Array<TransportadorModel>;
  ...
  loadData(nextPage?, callback?) {
    ...
    this.wbService.getTransportadores(params, nextPage).subscribe( response => {    
      if(!this.lista) this.lista = [];
      this.lista = this.lista.concat(response.data as Array<TransportadorModel>);
      ...
      buttons: [
        {
          ...
          handler: () => {
            ...
            let transportador = this.lista[index];
            this.wbService.deleteTransportador(transportador.id).subscribe( () => {    
  ...
  async loadModal(transportador:TransportadorModel, index?:number) {
    const modal = await this.modalCtrl.create({
      component: FormTransportadoresComponent,
      componentProps: {transportador: transportador}
    });
    modal.present();
      ...
      buttons: [
        {
          text: 'Novo',
          handler: (data) => {
            let transportador = new TransportadorModel();
            this.loadModal(transportador);
          }
        }, { 
          text: 'Buscar',
          handler: (data) => {
            ...
            this.wbService.presentLoading();
            this.wbService.getPessoaDocumento('transportador',data.documento).subscribe( response => {    
              let transportador = new TransportadorModel();
              transportador.pessoa = (response.pessoa as PessoaModel);
              this.loadModal(transportador);
              this.wbService.dismissLoading();
              form.dismiss();
            } , response => {
